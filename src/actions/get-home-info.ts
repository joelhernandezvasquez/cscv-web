...
export const getHomeInfo = async ():Promise<HomeInfo>=>{
  try{
    ...
    if(!request.ok){
     ...
     const {title,description,cover,about,activities,events,testimonies,ministerios} = response.data;
     const image = `${STRAPI_HOST}/${cover.url}`;
     
     return {title,description,image,about,activities,events,testimonies,ministerios};
